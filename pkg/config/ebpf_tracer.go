...
func (c *EBPFTracer) Validate() error {
	...
	if c.WakeupLen < 0 {
		return fmt.Errorf("ebpf.wakeup_len in the YAML configuration file or OTEL_EBPF_BPF_WAKEUP_LEN must be at least 1")
	}
	if c.BatchLength < 1 {
		return fmt.Errorf("ebpf.batch_length in the YAML configuration file or OTEL_EBPF_BPF_BATCH_LENGTH must be at least 1")
	}

	if c.BatchTimeout <= 0 {
		return fmt.Errorf("ebpf.batch_timeout in the YAML configuration file or OTEL_EBPF_BPF_BATCH_TIMEOUT must be greater than 0")
	}

	if c.HTTPRequestTimeout < 0 {
		return fmt.Errorf("ebpf.http_request_timeout in the YAML configuration file or OTEL_EBPF_BPF_HTTP_REQUEST_TIMEOUT must be greater than or equal to 0")
	}

	if !c.TCBackend.Valid() {
		return fmt.Errorf("invalid ebpf.traffic_control_backend in the YAML configuration file or OTEL_EBPF_BPF_TC_BACKEND value, must be 'tc' or 'tcx' or 'auto'")
	}
	...
	if c.ContextPropagationEnabled && c.ContextPropagation != ContextPropagationDisabled {
		return fmt.Errorf("ebpf.enable_context_propagation and ebpf.context_propagation in the YAML configuration file or OTEL_EBPF_BPF_ENABLE_CONTEXT_PROPAGATION and OTEL_EBPF_BPF_CONTEXT_PROPAGATION are mutually exclusive")
	}
	...
	if c.MySQLPreparedStatementsCacheSize <= 0 {
		return fmt.Errorf("ebpf.mysql_prepared_statements_cache_size in the YAML configuration file or OTEL_EBPF_BPF_MYSQL_PREPARED_STATEMENTS_CACHE_SIZE must be greater than 0")
	}

	if c.PostgresPreparedStatementsCacheSize <= 0 {
		return fmt.Errorf("ebpf.postgres_prepared_statements_cache_size in the YAML configuration file or OTEL_EBPF_BPF_POSTGRES_PREPARED_STATEMENTS_CACHE_SIZE must be greater than 0")
	}

	if c.KafkaTopicUUIDCacheSize <= 0 {
		return fmt.Errorf("ebpf.kafka_topic_uuid_cache_size in the YAML configuration file or OTEL_KAFKA_TOPIC_UUID_CACHE_SIZE must be greater than 0")
	}

	if c.MongoRequestsCacheSize <= 0 {
		return fmt.Errorf("ebpf.mongo_requests_cache_size in the YAML configuration file or OTEL_EBPF_BPF_MONGO_REQUESTS_CACHE_SIZE must be greater than 0")
	}
...
func (r RedisDBCacheConfig) Validate() error {
	if r.MaxSize <= 0 {
		return fmt.Errorf("ebpf.redis_db_cache.max_size in the YAML configuration file or OTEL_EBPF_BPF_REDIS_DB_CACHE_MAX_SIZE must be greater than 0")
	}
	return nil
}
