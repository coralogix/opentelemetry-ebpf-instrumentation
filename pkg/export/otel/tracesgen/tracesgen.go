...
// redactKeys is the list of extra query-parameter keys to scrub beyond the
// built-in OTel semconv set (DefaultRedactQueryParams), which is always applied.
...
// DefaultRedactQueryParams are always applied; pass additional redactKeys to
// extend the redact set. When processing many spans in a batch, prefer
// GroupSpans, which pre-builds the redact set once for the whole batch.
