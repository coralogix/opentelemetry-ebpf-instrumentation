...
type NetworkConfig struct {
	...
	ListenPollPeriod time.Duration `yaml:"listen_poll_period" env:"OTEL_EBPF_NETWORK_LISTEN_POLL_PERIOD" validate:"listenPollPeriod"`
...
func ValidateListenPollPeriod(fl validator.FieldLevel) bool {
	networkConfig, ok := fl.Parent().Interface().(NetworkConfig)
	if !ok {
		// it should not happen
		return false
	}
	if networkConfig.ListenInterfaces == NetworkListenInterfacesWatch && networkConfig.ListenPollPeriod != 0 {
		return false
	}
	if networkConfig.ListenInterfaces == NetworkListenInterfacesPoll && networkConfig.ListenPollPeriod <= 0 {
		return false
	}
	return true
}
