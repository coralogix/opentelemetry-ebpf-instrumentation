...
func rdlog() *slog.Logger {
	return slog.With("component", "stats.ReverseDNS")
}
...
func ReverseDNSProvider(cfg *ReverseDNS, input, output *msg.Queue[[]*ebpf.Stat]) swarm.InstanceFunc {
	return func(ctx context.Context) (swarm.RunFunc, error) {
		...
		log := rdlog()
		in := input.Subscribe(msg.SubscriberName("stats.ReverseDNS"))
...
func optGetName(log *slog.Logger, cache *expirable.LRU[string, string], ip string) string {
	if host, ok := cache.Get(ip); ok {
		return host
	}
	if names, err := netLookupAddr(ip); err == nil && len(names) > 0 {
		cache.Add(ip, names[0])
		return names[0]
	} else if err != nil {
		log.Debug("error trying to lookup by IP address", "ip", ip, "error", err)
	}
