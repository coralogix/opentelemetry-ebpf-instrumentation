...
// structFieldSize is sizeof(runtime.structField) on 64-bit (Go 1.21+):
// {*name, *_type, uintptr}.
const structFieldSize = 24

// Fields returns the fields of t in declaration order, or nil if t is not a
// struct. Skips fields whose type metadata was dropped by the linker.
func (t *Type) Fields() []Field {
	if t.Kind != Struct {
		return nil
	}
	// structType extension after rtType: PkgPath name (8B) + Fields slice
	// header (24B: data, len, cap). Skip past PkgPath, decode the header.
...
// readFieldName decodes a structField name at va (absolute, not a typesBase
// offset). The flags byte's bit 3 marks an embedded field. Returns name and
// embedded flag.
...
// Elem returns the element type for Pointer/Slice/Chan/Array (the first
// *_type in the kind extension). Nil for any other kind.
