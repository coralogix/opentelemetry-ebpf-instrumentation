...
// Package gometa decodes Go runtime type metadata (.typelink → _type → funcType,
// structType, ...) from a stripped Go binary, with no DWARF or symbol table.
// Targets Go 1.21+ on amd64/arm64 (64-bit little-endian).
...
// Walker holds parsed type metadata for one binary.
type Walker struct {
	rdata     []byte
	rdataAddr uint64
	typelinks []int32
...
// Open parses the type tables of a Go ELF. The returned walker holds slices
// of .rodata bytes; the *elf.File may be closed after Open returns.
...
// TypeByName returns the first type whose runtime name equals name (reflect
// convention: "main.Order", "*main.checkout", "[]string", ...). Returns nil
// when no reachable type matches.
...
// Types yields every reachable type once: typelink entries plus their closure
// through Elem (ptr/slice/chan/array) and struct fields. Yield returns false
// to stop early.
...
// findTypesBase resolves the moduledata.types address by scoring page-aligned
// candidates inside .rodata against the typelink table: a correct base makes
// every offset decode as a plausible _type record.
...
// rdataSlice returns n bytes at virtual address va, or false if va..va+n
// falls outside .rodata.
