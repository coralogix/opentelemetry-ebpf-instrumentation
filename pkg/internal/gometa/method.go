...
type Method struct {
	...
	// EntryPC is the absolute VA of the direct-call entry (textBase + Tfn).
	// Zero when the linker stripped the body (tfn == -1) or .text is absent.
	EntryPC uint64

	// FuncType is the decoded signature. Nil when the linker dropped the
	// method's type metadata — typical for unexported methods on types not
	// reached through reflect.
	FuncType *FuncType

	// TextOff is the raw int32 textOff stored on disk (-1 means stripped).
	TextOff int32
}

// FuncType is a decoded signature in declaration order.
...
const (
	uncommonTypeSize = 16 // sizeof(uncommonType) on 64-bit (Go 1.21+)
	methodEntrySize  = 16 // 4 × int32 per method record
	funcTypeHeader   = 8  // InCount + OutCount + pad following rtType
)

// Methods returns the method table in declaration order; nil when the type
// has no uncommonType block.
func (t *Type) Methods() []Method {
	...
	for i := uint16(0); i < mcount; i++ {
		...
		// Method record: name, mtyp, ifn, tfn (all int32). Ifn (mb[8:12])
		// is the interface-call entry; we only need the direct entry tfn.
...
// uncommonVA returns the VA of the uncommonType following t's extension.
...
// funcTypeAt decodes a funcType at va; nil if va is not a Func _type.
