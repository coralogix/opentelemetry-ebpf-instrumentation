...
func TestBasicPipeline(t *testing.T) {
	...
	assert.Equal(t, collector.MetricRecord{
		...
		ResourceAttributes: map[string]string{
			...
			string(semconv.ServiceNameKey):          "foo-svc",
			string(semconv.ServiceNamespaceKey):     "ns",
			string(semconv.ServiceVersionKey):       "v0.0.1",
...
func newRequest(serviceName string, path string, status int) []request.Span {
	return []request.Span{{
		...
		RequestStart: 1,
		End:          3,
		Service:      svc.Attrs{HostName: "the-host", UID: svc.UID{Namespace: "ns", Name: serviceName, Version: "v0.0.1"}, SDKLanguage: svc.InstrumentableGolang},
	}}
}
