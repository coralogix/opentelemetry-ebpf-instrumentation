...
// AutoAttrSlot binds one BPF event slot to its emitted attribute name and
// type. Produced by BuildFunctionAutoSpec; consumed by the span reader to
// label attributes on the outgoing span.
...
// ErrAutoAttrsUnsupported wraps every failure path of BuildFunctionAutoSpec
// — non-Go binary, unrecognised symbol shape, receiver type missing from
// .typelink, or a linker-dropped funcType.
var ErrAutoAttrsUnsupported = errors.New("custom_span: auto_attrs target not supported")

// MergeManualOverAuto overlays a manual `attrs:`-derived spec on top of an
// auto-derived spec. Manual entries win on argument-index collision (the user
// supplied a name and type, so their declaration is authoritative); the
// merged slot list drops any auto slots that the user has now claimed.
//
// The returned ArgCount is the highest index either side populated, so the
// BPF runtime walks the union.
...
// BuildFunctionAutoSpec resolves span.On.FunctionSpan in ef and synthesizes
// an extraction spec covering every primitive scalar / string argument and
// every primitive field reachable from a struct-pointer argument.
//
// Returns ErrAutoAttrsUnsupported (wrapped) for any failure. Callers should
// fall back to the manual-attrs path on error; the probe itself can still
// attach for timing-only spans.
...
// compileRecipes translates gometa recipes into BPF spec entries plus the
// ordered slot table the userspace reader uses for attribute names.
...
// parseGoMethodSymbol decodes the "<pkg>.(*<Type>).<Method>" symbol form —
// the only form supported by auto_attrs. pkg may contain slashes (vendored
// import paths). Value-receiver methods and plain top-level functions are
// rejected: the former are ambiguous to parse without a typelinks pass,
// the latter have no funcType retained in the runtime type table.
