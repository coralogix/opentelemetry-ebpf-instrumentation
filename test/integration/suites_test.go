...
func TestSuite_JavaKafka_280(t *testing.T) {
	compose, err := docker.ComposeSuite("docker-compose-java-kafka.yml", path.Join(pathOutput, "test-suite-java-kafka.log"))
	compose.Env = append(compose.Env, `OTEL_EBPF_OPEN_PORT=8080`, `OTEL_EBPF_EXECUTABLE_PATH=`, `TEST_SERVICE_PORTS=8381:8080`)
	require.NoError(t, err)
	require.NoError(t, compose.Up())
	t.Run("Java Kafka 2.8.0 tests", testJavaKafka)
	require.NoError(t, compose.Close())
}

func TestSuite_JavaKafka_400(t *testing.T) {
	compose, err := docker.ComposeSuite("docker-compose-java-kafka-400.yml", path.Join(pathOutput, "test-suite-java-kafka-400.log"))
	compose.Env = append(compose.Env, `OTEL_EBPF_OPEN_PORT=8080`, `OTEL_EBPF_EXECUTABLE_PATH=`, `TEST_SERVICE_PORTS=8381:8080`)
	require.NoError(t, err)
	require.NoError(t, compose.Up())
	t.Run("Java Kafka 4.0.0 tests", testJavaKafka400)
	require.NoError(t, compose.Close())
}
