...
const (
	...
	// /stop endpoint on the test host. POSTing it stops weaver and returns the
	// live-check report in the response body (weaver runs with `--output http`).
	// Every group is expected to wire weaver (append
	// `weaver/docker-compose-weaver.yml` to the test case's compose file list);
	// an unreachable admin port fails the spec so a new group can't silently
	// skip semantic-convention validation.
