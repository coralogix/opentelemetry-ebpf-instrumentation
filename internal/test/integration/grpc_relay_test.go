...
const (
	...
	// Longer timeout for gRPC relay — 8 services need time to start and propagate.
	// Java instrumentation (JVM attach) and 7-hop chain discovery can take
	// longer than 4 minutes in CI; 5 minutes gives sufficient headroom.
	grpcRelayTimeout = 5 * time.Minute
)
...
func testGRPCRelayChainContextPropagation(t *testing.T) {
	...
	// A fixed trace ID is used for the duration of the retry loop so that spans
	// from successive requests accumulate under one trace. Services come online
	// gradually (JVM attach, connection warm-up) and the full 7-hop chain only
	// becomes visible in Jaeger once every hop has been instrumented; accumulation
	// is essential for that build-up. Each request still uses a fresh span ID to
	// avoid stale cp_support_connect_info entries on reused connections.
	now0 := uint64(time.Now().UnixNano())
	relayAttemptTraceID := fmt.Sprintf("%016x%016x", now0, now0+1)
	var trace jaeger.Trace
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		traceparent := fmt.Sprintf("00-%s-%016x-01", relayAttemptTraceID, uint64(time.Now().UnixNano()))
	...
	if len(terminalSpans) > 0 {
		...
		for depth, span := range chain {
			...
			t.Logf("%s[%s]%s trace_id=[%s] span_id=[%s]", strings.Repeat("  ", depth), svcName, kind, span.TraceID, span.SpanID)
		}
	}
}
