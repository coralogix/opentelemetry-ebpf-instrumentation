...
var weaverIgnoredAdviceMessages = map[string]struct{}{
	...
	// `obi.test.name` is injected by the integration-test otelcol's
	// `resource/test` processor (see `otelcol-config-weaver*.yml`) so
	// shard-end weaver findings can be grouped by the originating test.
	// It's not part of the OBI registry by design — it's a test-only
	// transport-layer tag — so we suppress the registry-miss advice.
	"Attribute 'obi.test.name' does not exist in the registry.":             {},
	"Attribute name 'obi.test.name' collides with existing namespace 'obi'": {},
}
