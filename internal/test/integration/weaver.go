...
// This is the entry point for ~all existing docker-compose suites and must be
// called while the Docker Compose stack is still running.
func runWeaverValidation(t *testing.T) {
	t.Helper()
	runWeaverValidationDocker(t, false /* observeOnly */)
}

// runWeaverValidationObserve is the observe-only counterpart of
// runWeaverValidation for the Docker transport: it stops weaver and logs the
// full advisory breakdown (including which advisories WOULD fail under
// enforce mode) but never fails the test. Use this for NEW suites that are
// being brought online observe-first, before their emitted-attribute set has
// been declared in `schemas/obi/`.
func runWeaverValidationObserve(t *testing.T) {
	t.Helper()
	runWeaverValidationDocker(t, true /* observeOnly */)
}

// runWeaverValidationDocker is the shared Docker-transport driver: fetch the
// report from the weaver container, then validate it in the requested mode.
func runWeaverValidationDocker(t *testing.T, observeOnly bool) {
	t.Helper()
