...
//   - "undefined_enum_variant": an enum attribute emitted with a value that is
//     not a documented member of the enum (e.g. a new value OBI adds for a
//     known semconv enum). Weaver classifies these as `information`-level.
//     OBI requires every emitted enum value to be either a documented semconv
//     member or declared as an OBI extension member in `schemas/obi/` — so a
//     new/undocumented value must fail rather than silently pass.
//
