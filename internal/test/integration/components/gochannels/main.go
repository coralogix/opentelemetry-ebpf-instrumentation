...
func httpHandlers(mux *http.ServeMux) {
	...
	// Control pair: identical to /send and /recv but without the reschedule, to
	// tell a correlation that the reschedule defeats apart from one that never
	// works.
	mux.HandleFunc("/plain-send", func(w http.ResponseWriter, _ *http.Request) {
		plain <- job{id: 7}
		fmt.Fprint(w, "plain-sent")
	})
	mux.HandleFunc("/plain-recv", func(w http.ResponseWriter, _ *http.Request) {
		j := <-plain
		fmt.Fprintf(w, "plain-recv %d", j.id)
	})
