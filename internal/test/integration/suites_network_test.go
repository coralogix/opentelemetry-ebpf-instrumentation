...
func TestNetwork_Deduplication(t *testing.T) {
	compose := docker.SuiteStackServices(t, docker.NewStack(map[string]*docker.ServiceDef{
		...
		"jaeger":  nil,
	}), "docker-compose-netolly.yml")
	compose.Env = append(compose.Env, "OTEL_EBPF_NETWORK_DEDUPER=first_come")
...
func TestNetwork_Deduplication_Use_Socket_Filter(t *testing.T) {
	...
	compose.Env = append(compose.Env, "OTEL_EBPF_NETWORK_DEDUPER=first_come", "OTEL_EBPF_NETWORK_SOURCE=socket_filter")
	require.NoError(t, compose.Up())
...
func TestNetwork_NoDeduplication(t *testing.T) {
	...
	compose.Env = append(compose.Env, "OTEL_EBPF_NETWORK_DEDUPER=none")
	require.NoError(t, compose.Up())
	...
	compose := docker.SuiteStackServices(t, docker.NewStack(map[string]*docker.ServiceDef{
		"obi": docker.NewOBI(docker.OBI{
			...
			Env: map[string]string{
				...
				"OTEL_EBPF_OTLP_TRACES_BATCH_TIMEOUT":    "1ms",
				"OTEL_EBPF_BPF_BATCH_TIMEOUT":            "1s",
				"OTEL_EBPF_CONFIG_PATH":                  "/configs/obi-config-netolly-disallowattrs.yml",
				"OTEL_EBPF_NETWORK_CACHE_ACTIVE_TIMEOUT": "1s",
				"OTEL_EBPF_NETWORK_DEDUPER":              "${OTEL_EBPF_NETWORK_DEDUPER}",
				"OTEL_EBPF_NETWORK_METRICS":              "true",
				"OTEL_EBPF_NETWORK_PRINT_FLOWS":          "true",
...
func TestNetwork_ReverseDNS(t *testing.T) {
	compose := docker.SuiteStackServices(t, docker.NewStack(map[string]*docker.ServiceDef{
		...
		"otelcol": docker.OtelcolNoJaeger(),
		"jaeger":  nil,
	}), "docker-compose-netolly-rdns.yml")
	require.NoError(t, compose.Up())
	...
	compose := docker.SuiteStackServices(t, docker.NewStack(map[string]*docker.ServiceDef{
		"obi": docker.NewOBI(docker.OBI{
			...
			Env: map[string]string{
				...
				"OTEL_EBPF_OTLP_TRACES_BATCH_TIMEOUT":    "1ms",
				"OTEL_EBPF_BPF_BATCH_TIMEOUT":            "1s",
				"OTEL_EBPF_CONFIG_PATH":                  "/configs/obi-config-netolly-direction.yml",
		...
		"otelcol": docker.OtelcolNoJaeger(),
		"jaeger":  nil,
	}), "docker-compose-netolly-direction.yml")
	compose.Env = append(compose.Env, "OTEL_EBPF_NETWORK_DEDUPER=first_come", "OTEL_EBPF_NETWORK_SOURCE=tc")
	require.NoError(t, compose.Up())
