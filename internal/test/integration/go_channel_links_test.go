...
func TestSuite_GoChannelLinks(t *testing.T) {
	...
	waitForChanLinkTelemetry(t)

	t.Run("plain handoff, no reschedule", testChannelLinkPlain)
...
// waitForChanLinkTelemetry blocks until OBI has attached and exported at least
// one span. Without it the first subtest drives its requests inside the attach
// window and sees no telemetry at all, which looks identical to a missing link.
func waitForChanLinkTelemetry(t *testing.T) {
	t.Helper()

	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		ti.DoHTTPGet(t, chanLinksHTTPURL+"/health", http.StatusOK)

		traces, err := fetchChanLinkTraces("GET /health")
		require.NoError(ct, err)
		require.NotZero(ct, countSpans(traces, "GET /health"),
			"OBI has not exported any span yet")
	}, testTimeout, time.Second)
}

// Control for the reschedule: the handlers touch the channel immediately, so a
// failure here and a pass in the rescheduled cases would isolate the reschedule
// as the thing defeating correlation, and vice versa.
func testChannelLinkPlain(t *testing.T) {
	for range 5 {
		driveHandoff(t, chanLinksHTTPURL, "/plain-recv", "/plain-send")
	}
	requireLinkedReceiver(t, "GET /plain-recv")
}
...
// observedOperations lists every operation name the service actually produced,
// so a failed assertion says what arrived instead of only what was missing.
func observedOperations() string {
	params := url.Values{
...
func requireLinkedReceiver(t *testing.T, recvOperation string) {
	...
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		require.NotZero(ct, countSpans(traces, recvOperation),
			"no %q span reached the collector; observed operations: %s",
			recvOperation, observedOperations())
...
func requireNoLinks(t *testing.T, operation string) {
	...
	require.NotZero(t, countSpans(traces, operation),
		"no %q span reached the collector, so this proves nothing; observed operations: %s",
		operation, observedOperations())
