...
static void test_selected_process_is_cached_by_host_pid(void) {
    ...
    check_bool("the positive entry is keyed by the host pid", true, test_cache_find(12345) != NULL);
    check_bool("no entry is keyed by the namespaced pid", true, test_cache_find(7) == NULL);
...
static void test_unselected_process_takes_the_slow_path_once(void) {
    ...
    check_bool("the negative entry is keyed by the host pid", true, e != NULL);
...
static void test_parent_authorized_later_is_reevaluated_after_clear(void) {
    ...
    check_bool("the worker is now cached positive", true, test_cache_find(600) != NULL);
}
...
static void test_filter_off_accepts_everything(void) {
    ...
    check_bool("and without touching the cache", true, test_cache_find(800) == NULL);
}
...
static void test_unreadable_pid_is_not_cached(void) {
    ...
    check_bool("but not cached, so it is retried", true, test_cache_find(900) == NULL);
}
