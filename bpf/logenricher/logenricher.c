...
static __always_inline int __write(struct kiocb *iocb, struct iov_iter *from, const int fd) {
    ...
    if (e->len > 0) {
        ...
        if (err < 0) {
            bpf_dbg_printk("logenricher: failed to write log event to ringbuf");
            return 0;
        }
        ...
        if (zeroed_count) { // this is never zero, but the verifier is not smart enough
            bpf_probe_write_user(iov.iov_base, zero, zeroed_count);
        }
    }
...
int BPF_KPROBE(obi_kprobe_ksys_write, unsigned int fd) {
    ...
    if (bpf_map_update_elem(&pid_fd, &(u64){bpf_get_current_pid_tgid()}, (int *)&fd, BPF_ANY)) {
        bpf_dbg_printk("logenricher: ksys_write: failed to update pid_fd map");
    }
