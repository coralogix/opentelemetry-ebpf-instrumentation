...
resolve_path(char *buf, struct path *path, const struct task_struct *task) {
    ...
    if (!(dentry_arr = bpf_map_lookup_elem(&path_resolver_scratch, &(u32){0}))) {
        bpf_dbg_printk("%s: could not get path resolver scratch area", __FUNCTION__);
        goto out_err;
    }
    ...
    for (int i = k_path_resolver_max_components - 1; i >= 0; i--) {
        ...
        if (size + component.len + 1 > k_pts_file_path_len_max) {
            bpf_dbg_printk("%s: path under construction is too long: %s", __FUNCTION__, buf);
            goto out_err;
        }
        ...
        if (ret > 0) {
            ...
            bpf_dbg_printk("%s: could not read d_name at %p, current path %s",
                           __FUNCTION__,
                           component.name,
                           buf);
            goto out_err;
