...
int obi_uprobe_runtime_casgstatus(struct pt_regs *ctx) {
    ...
    // NOTE: these runtime types are somehow special and don't
    // appear in go:itab. These structs appear to be stable since
    ...
    //  ...
    // }
    const u8 k_g_m_off = 0x30;
    const u8 k_m_procid_off = 0x40;
