...
| `metrics.disable_target_info` | `boolean` | `OTEL_EBPF_METRICS_DISABLE_TARGET_INFO` | `false` |  |  | Stops OBI from exporting its own target.info metric, for collectors that already synthesize target_info from resource attributes when re-exporting in Prometheus format. traces.target.info and traces.host.info have no counterpart and are unaffected. |
