...
- **Go uprobe** (`grpcFramerWriteHeaders`) — definitive trace from Go runtime
- **kprobe CLIENT** (`http2_grpc_start`) — for non-Go services, or overridden by `adopt_injected_trace` when Go uprobes wrote first

`adopt_injected_trace`: called after `find_trace_for_client_request` in the kprobe CLIENT path. Overrides stale traces with the Go uprobe's entry from `outgoing_trace_map[{ports, stream_id}]`.
