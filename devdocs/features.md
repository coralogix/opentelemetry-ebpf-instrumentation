...
| Aerospike     |    All    |         All | GET, EXISTS, PUT, TOUCH, OPERATE, DELETE, SCAN, QUERY, BATCH, UDF                         |   No   |                 No |     Native client protocol (port 3000); TLS and compressed (type-4) payloads not parsed; only operation metadata captured, not record/bin values; `db.query.text` requires the client's `sendKey` write policy (otherwise only the key digest is on the wire)
