...
`tokio::runtime::blocking::pool::spawn_blocking` is the inner free function that the public `tokio::task::spawn_blocking` wrapper delegates to. The wrapper is minlined away in release; the inner function survives in both builds, so it is the symbol probed.

`obi_uretprobe_tokio_spawn_blocking` reads the returned `JoinHandle` (the new blocking task pointer) from the return register, finds the handler task still "current" on this thread, and copies the handler's connection onto the blocking mtask with `conn_valid = 1`. 
The egress walk then resolves the blocking task at depth 0 without needing the racy process-level fallback.

## Parent Lookup

When a Tokio client request needs a trace parent (at egress, via `find_tokio_parent_trace_for_thread` in `trace_parent.h`), lookup happens in two mphases plus fallbacks.
