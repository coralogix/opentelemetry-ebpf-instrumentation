...
func TestParseDenominatorSplitsSurfacesAndExcludesSpans(t *testing.T) {
	resolved := []byte(`{"groups":[
		...
		{"type":"metric","metric_name":"target.info","attributes":[
			{"name":"service.name"},{"name":"k8s.pod.name"}]},
		{"type":"entity","name":"cloud","attributes":[{"ref":"cloud.provider"}]},
		{"type":"span","span_kind":"client","attributes":[{"ref":"url.full"}]},
	...
	// Every metric OBI emits — the target.info carrier and OBI-internal metrics
	// included — counts as a metric name.
	assert.Equal(t, []string{
		"http.server.request.duration", "obi.internal.build.info",
		"obi.network.flow.bytes", "target.info",
	}, got.MetricNames)
	// Metric attributes are the data-point attributes of non-carrier metric groups.
	assert.Equal(t, []string{"http.request.method", "server.address", "src.address"}, got.MetricAttributes)
	// Resource attributes are the target.info carrier labels plus entity attributes.
	assert.Equal(t, []string{"cloud.provider", "k8s.pod.name", "service.name"}, got.ResourceAttributes)
	// Span attributes (OTEP #5233: not coverable) and bare attribute_group
	// definitions are excluded from every surface.
	assert.NotContains(t, got.MetricAttributes, "url.full")
	assert.NotContains(t, got.ResourceAttributes, "url.full")
	assert.NotContains(t, got.MetricAttributes, "url.path")
}
